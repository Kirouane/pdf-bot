...
describe('queue : retrieval', function () {
    ...
    it('should return the next job if no tries were found', function () {
        ...
        var job = queue.getNext(function () {
        }, 5)
    ...
    it('should get next with no pings', function () {
        ...
        var job = queue.getNextWithoutSuccessfulPing(function () {
            return 1000 * 60 * 4
        }, 5)
    ...
    it('should get next ping that is within decay schedule', function () {
        ...
        queue = createQueue({}, [
            // not within decay
            Object.assign(createJob(true), {
                pings: [{id: 1, error: true, sent_at: dateTwo}, {
                    id: 2,
                    error: true,
                    sent_at: dateTwo
                }]
    ...
    it('should set busy with date', function(done){
        queue = createQueue({}, [
            createJob(true)
        ])
        ...
        db = getDb()
        console.log(db)
        assert.equal(db.is_busy, false)
        assert.equal(db.is_busy_datetime.substr(0, 10), (new Date()).toISOString().substr(0, 10))

        queue.setIsBusy(true)
        db = getDb()
        assert.equal(db.is_busy, true)
        assert.equal(db.is_busy_datetime.substr(0, 10), (new Date()).toISOString().substr(0, 10))

        done()
    })

    it('should is busy with date', function(done){
        queue = createQueue({}, [
            createJob(true)
        ])
